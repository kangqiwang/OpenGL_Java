...
public class Transform {
	...
	public void scale(float sx, float sy, float sz){
		// Complete this function to implement scaling transformation.
		// The action here is to modify the matrix T, so that
		// the modified T is the result of T PRE-MULTIPLIED by
		// the scaling matrix with scaling factors (sx, sy, sz) 
		// Refer to the lecture notes for the defineition of 
		// scaling matrix. 
	}
	...
	public void rotateX(float angle){
		...
		float ca = (float) Math.cos(angle*Math.PI/180);
		float sa = (float) Math.sin(angle*Math.PI/180);
		// Complete this function to implement rotation around X-axis.
		// The action here is to modify the matrix T, so that
		// the modified T is the result of T PRE-MULTIPLIED by
		// the rotation matrix with rotation around X-axis by angle.
		// Please refer to the following methods rotateY() or rotateZ()   
	}		
